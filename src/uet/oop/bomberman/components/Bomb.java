...
public class Bomb extends Component {
    ...
    public Bomb() {
        onCollisionEnd(BBMType.BOMB, BBMType.PLAYER, (b, p) -> {
            if (entity != null) {
                wallBomb = spawn("wall_bomb", new SpawnData(entity.getX(), entity.getY()));
            }
        });

        animation = new AnimationChannel(image(spriteSheetChosen), 16, SIZE_BLOCK, SIZE_BLOCK,
    ...
    public void getRightBlocks() {
        List<Entity> temp = new ArrayList<>(getGameWorld().getEntitiesByType(BBMType.WALL));
